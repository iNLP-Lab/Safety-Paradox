#!/bin/bash
# GRPO online RL training for red-teaming classifier (full-model, no PEFT)
# Usage: bash train/grpo_rl.sh
#
# Step 1: Prepare GRPO dataset from wildguard_RL.csv
# Step 2: Run GRPO training

set -e

# Config: base_model can be passed as first arg, e.g. bash grpo_rl.sh meta-llama/Llama-3.1-8B-Instruct
base_model="${1:-meta-llama/Llama-3.1-8B-Instruct}"
model="${base_model##*/}"
dataset_name="SAI"

# FSDP config: match model family (no cpu_ram_efficient_loading)
# Qwen3 uses Qwen3DecoderLayer; Qwen2 uses Qwen2DecoderLayer — check Qwen3 first
# Falcon3 is Llama-based (LlamaDecoderLayer); older Falcon uses FalconDecoderLayer
case "${base_model}" in
...
echo ""
echo "=== Step 2: GRPO online RL training (full-model) ==="
